...
export default function ViewSubCategory() {
  ...
  const [limit,setLimit]=useState(0)
    ...
    let singleRowDelete=(id)=>{
      ...
      axios.delete(AdminBaseURL+`/sub-category/delete/${id}`)
            .then((res)=>{
      ...
      if(allCheckedId.length==0){
        ...
        const swalWithBootstrapButtons = Swal.mixin({
          ...
          swalWithBootstrapButtons
            ...
            .then((result) => {
              if (result.isConfirmed) {
                axios.post(AdminBaseURL+"/sub-category/multiple-delete",{ids:allCheckedId})
                .then((res)=>{
                    if(res.data.status===1){
                        console.log(res.data)
                        swalWithBootstrapButtons.fire({
    ...
    <table className="w-full  text-left rtl:text-right text-gray-500 ">
        ...
        <tbody>
            ...
            subCatData.map((item,index)=>{
                ...
                <td className="px-6 py-4">
                    <img className='w-16 h-16 rounded-md object-cover object-top' src={path+item.subCategoryImage} alt="" />
                </td>
                <td className="px-6 py-4">
